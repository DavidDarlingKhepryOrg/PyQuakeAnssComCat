...
pgm_name = 'QuakeAnssComCatRevGeoCoder.py'
pgm_version = '1.0'
...
arg_parser.add_argument('--src-file-path', required=False, help='source file path', default='~/Desktop/Quakes/ANSS_ComCat_Quakes_20200101_20201231.csv')
...
if os.path.exists(args.src_file_path):
    ...
    with io.open(args.out_file_path, 'w', newline='', encoding="utf8") as out_file:

        # open the source file for reading
        with io.open(args.src_file_path, 'r', newline='', encoding="utf8") as src_file:
            ...
            csv_writer = csv.DictWriter(out_file,
                                        ...
                                        quoting=args.out_quotemode_enm,
            ...
            for row in csv_reader:
                ...
                if row['Event_DTG'] is not None and row['Event_DTG'].endswith('Z'):
                    row['Event_DTG'] = row['Event_DTG'][:-5].replace(args.src_date_ymd_separator, args.out_date_ymd_separator).replace('T', ' ')
                if row['Updated_DTG'] is not None and row['Updated_DTG'].endswith('Z'):
                ...
                if event_dtg != args.out_db_null_value:
                    ...
                    row['Event_Year'] = str(event_dtg.year)
                    row['Event_Month'] = str(event_dtg.month)
                    row['Event_Day'] = str(event_dtg.day)
                    row['Event_Hour'] = str(event_dtg.hour)
                    row['Event_Min'] = str(event_dtg.minute)
                    row['Event_Sec'] = str(event_dtg.second)
                    ...
                    # using a mode 1 (single-threaded) search as mode 2 seems to fail presently
